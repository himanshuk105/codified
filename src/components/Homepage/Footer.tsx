...
const Contactform = () => {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-950 via-black to-purple-950 h-[calc(100vh-100px)] text-white flex flex-col md:flex-row gap-6 p-8 rounded-3xl border border-white/10 shadow-inner">
      ...
      <form className="flex flex-col gap-3 p-5 border border-[#333] bg-[#111] rounded-xl w-full md:max-w-xl shadow-lg">
        {payloadform !== null && payloadform.length > 0 ? (
          payloadform.map((field, index) => (
            <InputField
              key={index}
    ...
    <div className="border border-[#222] w-full px-3 py-4 rounded-lg bg-black/20">
      {type === 'select' ? (
        <select
          ...
          className="w-full bg-transparent text-white placeholder-gray-400 focus:outline-none"
        >
          <option className="bg-[#222]">Select your Budget</option>
          {options &&
            options.map((fe: any, i: number) => (
              <option key={i} className="bg-[#222]" value={fe?.value}>
                {fe.label}
              </option>
            ))}
        </select>
