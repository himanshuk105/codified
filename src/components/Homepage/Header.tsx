...
//               {/* Dropdown */}
//               {hasDropdown && (
//                 <ul className="absolute right-0 mt-2 w-screen bg-white text-black opacity-0 group-hover:opacity-100 group-hover:translate-y-0 translate-y-2 transition-all duration-300 invisible group-hover:visible z-10">
...
const servicesDropdown = [
  ...
  {
    category: 'E-Commerce Development & Maintenance',
    items: ['Adobe Commerce (Magento)', 'Shopify & Shopify Plus', 'BigCommerce & WooCommerce'],
  },
  {
    ...
    items: [
      ...
      'Usability Testing & Performance Optimization',
    ],
  },
  {
    category: 'Hosting Cloud Services',
    items: ['Aws', 'Azure', 'Google'],
  },
]
          ...
          {nav.map(({ href, text, hasDropdown }) => (
            <li key={href} className="group">
              <Link
                href={href}
                className={`hover:text-blue-400 flex items-center gap-1 transition-all ${
                  pathname !== href ? 'text-white' : 'text-white/50 hover:text-white border-b'
                }`}
              >
                {text}
                {hasDropdown && <FaChevronDown className="text-xs" />}
              ...
              {/* Services Dropdown */}
              {hasDropdown && text === 'Services' && (
                <div className="absolute left-0 mt-3 bg-white text-black p-4 shadow-lg grid grid-cols-2 gap-4 z-50 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 w-screen">
                  {servicesDropdown.map((group, i) => (
                    <div key={i}>
              ...
              {/* Other Dropdowns */}
              {hasDropdown && text !== 'Services' && (
                <ul className="absolute left-0 mt-3 bg-white text-black p-4 shadow-lg grid grid-cols-2 gap-4 z-50 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-300 w-screen">
                  {(dropdownItems[text.toLowerCase() as keyof typeof dropdownItems] || []).map(
                    (item, i) => (
                      <li key={i} className="hover:bg-gray-100 px-4 py-2 cursor-pointer text-sm">
                        <Link
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ x: '100%' }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
            className="md:hidden fixed top-14 right-0 h-120 w-3/4 text-black p-6 z-50 bg-transparent"
          >
                  ...
                  {hasDropdown && openDropdown === text.toLowerCase() && (
                    <ul className="ml-4 mt-2 space-y-2 text-sm">
                      {text === 'Services'
                        ? servicesDropdown.map((group, i) => (
                            <li key={i}>
                              <p className="font-semibold mt-2">{group.category}</p>
                              <ul className="ml-2">
                                {group.items.map((item, idx) => (
                                  <li key={idx}>
                                    <Link
